...
export const realizarPago = async (req, res) => {
    ...
    try {
        ...
        const [carritoItems] = await connection.execute(
            `SELECT c.id_carrito, co.id_cotizacion, s.nombre_servicio, co.cantidad, co.valor
            FROM carrito c
            JOIN cotizaciones co ON c.id_cotizacion = co.id_cotizacion
        ...
        for (let item of carritoItems) {
            ...
            await connection.execute(
                `INSERT INTO reservas (id_usuario, id_carrito, fecha_reserva)
                 VALUES (?, ?, ?)`,
                [id_usuario, item.id_carrito, fechaReserva]
            );
        }
